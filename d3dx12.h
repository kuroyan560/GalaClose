...
struct CD3DX12_VIEWPORT : public D3D12_VIEWPORT
{
    ...
    explicit CD3DX12_VIEWPORT(
        ...
        FLOAT maxDepth = D3D12_MAX_DEPTH ) noexcept
    {
        auto DescHeap = pResource->GetDesc();
        const UINT64 SubresourceWidth = DescHeap.Width >> mipSlice;
        const UINT64 SubresourceHeight = DescHeap.Height >> mipSlice;
        switch (DescHeap.Dimension)
        {
        case D3D12_RESOURCE_DIMENSION_BUFFER:
            TopLeftX = topLeftX;
            TopLeftY = 0.0f;
            Width = float(DescHeap.Width) - topLeftX;
            Height = 1.0f;
            break;
...
struct CD3DX12_RESOURCE_BARRIER : public D3D12_RESOURCE_BARRIER
{
    ...
    {}
    static inline CD3DX12_RESOURCE_BARRIER Activity(
        _In_ ID3D12Resource* pResource,
...
struct CD3DX12_CPU_DESCRIPTOR_HANDLE : public D3D12_CPU_DESCRIPTOR_HANDLE
{
    ...
    static inline void InitOffsetted(_Out_ D3D12_CPU_DESCRIPTOR_HANDLE &texHandle, _In_ const D3D12_CPU_DESCRIPTOR_HANDLE &base, INT offsetScaledByIncrementSize) noexcept
    {
        texHandle.ptr = SIZE_T(INT64(base.ptr) + INT64(offsetScaledByIncrementSize));
    }

    static inline void InitOffsetted(_Out_ D3D12_CPU_DESCRIPTOR_HANDLE &texHandle, _In_ const D3D12_CPU_DESCRIPTOR_HANDLE &base, INT offsetInDescriptors, UINT descriptorIncrementSize) noexcept
    {
        texHandle.ptr = SIZE_T(INT64(base.ptr) + INT64(offsetInDescriptors) * INT64(descriptorIncrementSize));
    }
};
...
struct CD3DX12_GPU_DESCRIPTOR_HANDLE : public D3D12_GPU_DESCRIPTOR_HANDLE
{
    ...
    static inline void InitOffsetted(_Out_ D3D12_GPU_DESCRIPTOR_HANDLE &texHandle, _In_ const D3D12_GPU_DESCRIPTOR_HANDLE &base, INT offsetScaledByIncrementSize) noexcept
    {
        texHandle.ptr = UINT64(INT64(base.ptr) + INT64(offsetScaledByIncrementSize));
    }

    static inline void InitOffsetted(_Out_ D3D12_GPU_DESCRIPTOR_HANDLE &texHandle, _In_ const D3D12_GPU_DESCRIPTOR_HANDLE &base, INT offsetInDescriptors, UINT descriptorIncrementSize) noexcept
    {
        texHandle.ptr = UINT64(INT64(base.ptr) + INT64(offsetInDescriptors) * INT64(descriptorIncrementSize));
    }
};
...
{
    auto DescHeap = pDestinationResource->GetDesc();
    UINT64 RequiredSize = 0;

    ID3D12Device* pDevice = nullptr;
    pDestinationResource->GetDevice(IID_ID3D12Device, reinterpret_cast<void**>(&pDevice));
    pDevice->GetCopyableFootprints(&DescHeap, FirstSubresource, NumSubresources, 0, nullptr, nullptr, nullptr, &RequiredSize);
    pDevice->Release();
    ...
    _In_reads_(NumSubresources) const D3D12_SUBRESOURCE_DATA* pSrcData) noexcept
{
    ...
    ID3D12Device* pDevice = nullptr;
    pDestinationResource->GetDevice(IID_ID3D12Device, reinterpret_cast<void**>(&pDevice));
    pDevice->GetCopyableFootprints(&DescHeap, FirstSubresource, NumSubresources, IntermediateOffset, pLayouts, pNumRows, pRowSizesInBytes, &RequiredSize);
    pDevice->Release();
    ...
    _In_reads_(NumSubresources) const D3D12_SUBRESOURCE_DATA* pSrcData) noexcept
{
    ...
    ID3D12Device* pDevice = nullptr;
    pDestinationResource->GetDevice(IID_ID3D12Device, reinterpret_cast<void**>(&pDevice));
    pDevice->GetCopyableFootprints(&DescHeap, FirstSubresource, NumSubresources, IntermediateOffset, Layouts, NumRows, RowSizesInBytes, &RequiredSize);
    pDevice->Release();
...
struct CD3DX12_PIPELINE_STATE_STREAM2
{
    ...
    CD3DX12_PIPELINE_STATE_STREAM2(const D3D12_GRAPHICS_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , InputLayout(DescHeap.InputLayout)
        , IBStripCutValue(DescHeap.IBStripCutValue)
        , PrimitiveTopologyType(DescHeap.PrimitiveTopologyType)
        , VS(DescHeap.VS)
        , GS(DescHeap.GS)
        , StreamOutput(DescHeap.StreamOutput)
        , HS(DescHeap.HS)
        , DS(DescHeap.DS)
        , PS(DescHeap.PS)
        , BlendState(CD3DX12_BLEND_DESC(DescHeap.BlendState))
        , DepthStencilState(CD3DX12_DEPTH_STENCIL_DESC1(DescHeap.DepthStencilState))
        , DSVFormat(DescHeap.DSVFormat)
        , RasterizerState(CD3DX12_RASTERIZER_DESC(DescHeap.RasterizerState))
        , RTVFormats(CD3DX12_RT_FORMAT_ARRAY(DescHeap.RTVFormats, DescHeap.NumRenderTargets))
        , SampleDesc(DescHeap.SampleDesc)
        , SampleMask(DescHeap.SampleMask)
        , CachedPSO(DescHeap.CachedPSO)
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM2(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , PrimitiveTopologyType(DescHeap.PrimitiveTopologyType)
        ...
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM2(const D3D12_COMPUTE_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , CS(CD3DX12_SHADER_BYTECODE(DescHeap.CS))
        , CachedPSO(DescHeap.CachedPSO)
    {
...
struct CD3DX12_PIPELINE_STATE_STREAM1
{
    ...
    // Mesh and amplification shaders must be set manually, since they do not have representation in D3D12_GRAPHICS_PIPELINE_STATE_DESC
    CD3DX12_PIPELINE_STATE_STREAM1(const D3D12_GRAPHICS_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , InputLayout(DescHeap.InputLayout)
        ...
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM1(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , PrimitiveTopologyType(DescHeap.PrimitiveTopologyType)
        ...
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM1(const D3D12_COMPUTE_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , CS(CD3DX12_SHADER_BYTECODE(DescHeap.CS))
...
struct CD3DX12_PIPELINE_MESH_STATE_STREAM
{
    ...
    CD3DX12_PIPELINE_MESH_STATE_STREAM(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , PS(DescHeap.PS)
...
struct CD3DX12_PIPELINE_STATE_STREAM
{
    ...
    CD3DX12_PIPELINE_STATE_STREAM(const D3D12_GRAPHICS_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , InputLayout(DescHeap.InputLayout)
        ...
        , CachedPSO(DescHeap.CachedPSO)
    {}
    CD3DX12_PIPELINE_STATE_STREAM(const D3D12_COMPUTE_PIPELINE_STATE_DESC& DescHeap) noexcept
        : Flags(DescHeap.Flags)
        , NodeMask(DescHeap.NodeMask)
        , pRootSignature(DescHeap.pRootSignature)
        , CS(CD3DX12_SHADER_BYTECODE(DescHeap.CS))
...
inline HRESULT D3DX12ParsePipelineStream(const D3D12_PIPELINE_STATE_STREAM_DESC& DescHeap, ID3DX12PipelineParserCallbacks* pCallbacks)
{
    ...
    if (DescHeap.SizeInBytes == 0 || DescHeap.pPipelineStateSubobjectStream == nullptr)
    {
    ...
    for (SIZE_T CurOffset = 0, SizeOfSubobject = 0; CurOffset < DescHeap.SizeInBytes; CurOffset += SizeOfSubobject)
    {
        BYTE* pStream = static_cast<BYTE*>(DescHeap.pPipelineStateSubobjectStream)+CurOffset;
